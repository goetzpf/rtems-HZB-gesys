...
#CFLAGS   += -O2
# undo optimization flags from $(RTEMS_ROOT)/make/custom/$(THE_BSP).cfg
# was: CFLAGS_OPTIMIZE_V = -O2 -g
CFLAGS_OPTIMIZE_V = 
# also override this from $(RTEMS_ROOT)/make/compilers/gcc-target-default.cfg
# was CFLAGS_DEFAULT = -g -Wall
...
USE_TECLA_YES_DEFINES  = -DWINS_LINE_DISC
...
# Thanks Till!! (again...)
# Extract '.gnu.attributes' section from ELF file.
# Produces c-file for compilation and linking with final executable.
...
define extract-objattrs
	if [ ! -x `which $(READELF)` ]; then (exit 126) fi && $(READELF) -x.gnu.attributes $@ 2> /dev/null | sed \
	-e '1i\/*\n * DO NOT EDIT THIS FILE -- IT WAS GENERATED AUTOMATICALLY\n * (dump of '.gnu.attributes' section)\n */\nstatic const unsigned char thebytes[] = {' \
    -e '$$a\};\nconst unsigned char *cexpSystemAttributes = thebytes;\nconst unsigned cexpSystemAttributesSize   = sizeof(thebytes);' \
    -e '/^[ \t]*\(0x[0-9a-fA-F]\{8\}\|[0-9a-fA-F]\{4\}\) /!d' \
    -e '/^[ \t]*\(0x[0-9a-fA-F]\{8\}\|[0-9a-fA-F]\{4\}\) /s/\(^[ \t]*\(0x[0-9a-fA-F]\{8\}\|[0-9a-fA-F]\{4\}\) \)\(.\{36\}\)\(.*\)/\3/g' \
...
endif #end ifdef XSYMS
endif #end ifdef ELFEXT
	$(OBJCOPY) --extract-symbol $@ $(@:%$(ELFEXT)=%sym)
else # USE_BUILTIN_SYMTAB=YES
	$(bsp-link-cxx)
	$(extract-objattrs)
	$(CC) -c -Wall $(CPU_CFLAGS) -o $(ARCH)/$(ATTRIBUTES).o $(ARCH)/$(ATTRIBUTES).c
	$(LINK.cc) $(CPU_CFLAGS) $(AM_CFLAGS) $(AM_LDFLAGS) \
	    -o $(basename $@).exe $(LINK_OBJS) $(ARCH)/$(ATTRIBUTES).o $(LINK_LIBS)
	$(bsp-post-link)
endif #end ifeq "$(USE_BUILTIN_SYMTAB)xx" "NOxx"
...
install: all $(RTEMS_SITE_INSTALLDIR)
ifdef RTEMS_SITE_INSTALLDIR
		$(INSTALL_VARIANT) $(INSTFILES) ${RTEMS_SITE_INSTALLDIR}
else
	@echo " "
	@echo "Must define RTEMS_SITE_INSTALLDIR for installation to proceed..."
	@echo "eg: make RTEMS_SITE_INSTALLDIR=/path/to/installdir install"
	@echo " "
endif
